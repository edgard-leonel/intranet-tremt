...
PROJECT_NAME=intranet-tremt
STACK_NAME=intranet-tre-mt-jus-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://intranet-tremt.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t edgard-leonel/intranet-tremt-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t edgard-leonel/intranet-tremt-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet-tremt-frontend-acceptance --link intranet-tremt-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d edgard-leonel/intranet-tremt-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-tremt-backend-acceptance -d edgard-leonel/intranet-tremt-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop intranet-tremt-frontend-acceptance
	@docker stop intranet-tremt-backend-acceptance
